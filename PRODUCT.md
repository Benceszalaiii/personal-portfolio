...
1. **The site is the portfolio.** Every pixel is a work sample. Craft, performance, and polish are the argument for hiring; a rough edge here reads as a rough edge in client work.
2. **Restraint is the flex.** Premium comes from control, not volume. Keep the signature effects (plasma, 3D, motion) but let each breathe — one deliberate moment beats five competing ones.
3. **Luxury through cohesion.** A single committed identity (dark-red on charcoal) applied consistently reads as expensive; a grab-bag of accent colors and fonts reads as a template.
4. **Earn the contact.** Guide the visiting business from "who is this" → "look what he made" → "here's how to reach him." The path to contact should never be a puzzle.
5. **Show, don't tell.** Prove skill by demonstrating it (a smooth 3D scene, a flawless morph transition) rather than listing adjectives about it.

## Accessibility & Inclusion

Target **WCAG 2.1 AA** for the fundamentals: body text ≥ 4.5:1 and large text ≥ 3:1 against the dark surfaces, fully keyboard-navigable, visible focus states, meaningful `alt` text and `aria-label`s (Hungarian). Motion is a deliberate selling point and is embraced rather than minimized — but it must still degrade gracefully under `prefers-reduced-motion` (crossfade/static-frame fallbacks) so the experience never breaks or ships content hidden for reduced-motion or headless renderers.

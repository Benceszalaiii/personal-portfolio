...
/*
  React Bits <LaserFlow /> adapted for this project:
  - "use client" + typed pointer handlers (no `as any` casts)
  - the canvas renders on opaque black; pair it with `mix-blend-screen` on the
    className so the beam composites onto the page's charcoal instead of
    painting a black rectangle
  - `color` is a plain hex prop: pass a resolved theme color (see
    useThemeColors) and the uniform updates live on tweakcn theme changes
*/
...
export const LaserFlow: FC<Props> = ({
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: props are read only as initial uniform values here; the second effect pushes live updates — re-running this one would rebuild the WebGL context
  useEffect(() => {

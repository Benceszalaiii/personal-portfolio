...
export interface LineSidebarProps {
  ...
  /** Controlled active index — when set, external changes (e.g. scroll-spy) sync the highlight */
  active?: number | null;
...
const LineSidebar = ({
  ...
  // Controlled mode: follow the parent's active index (scroll-spy etc.)
  useEffect(() => {
        ...
        {items.map((label, index) => (
          ...
          >
            ...
            <button
              ...
              className="relative inline-flex cursor-pointer items-baseline border-0 bg-transparent p-0 text-left leading-[1.2] outline-none focus-ember before:absolute before:-inset-x-12 before:-inset-y-[6px] before:content-[''] [color:color-mix(in_srgb,var(--accent-color)_calc(var(--effect,0)*100%),var(--text-color))] [font-size:var(--font-size)] [transform:translateX(calc(var(--effect,0)*var(--max-shift)))]"
            >
              {showIndex && (
